...
- **환자 기록은 공유 데이터베이스(Vercel Postgres)에 저장돼요.** 여러 직원 PC에서 같은
  기록을 검색할 수 있고, 저장 개수 제한도 없어요. 아래 순서대로 딱 한 번만 설정하면 돼요:
  1. Vercel 프로젝트 대시보드 상단 **Storage** 탭 클릭
  2. **Create Database** → **Postgres** 선택 → 이름 아무거나 입력 → 생성
  3. 생성된 DB에서 **Connect Project** 클릭 → 이 프로젝트 선택 → 연결
     (이 과정에서 `POSTGRES_URL` 같은 환경변수가 자동으로 추가돼요. 직접 입력할 값 없음)
  4. **Deployments** 탭 → 최근 배포 **⋯** → **Redeploy** (환경변수 반영을 위해 재배포 필요)
  - 테이블은 앱이 처음 저장을 시도할 때 자동으로 만들어져요. 별도로 SQL을 실행할 필요는 없어요.
  - Vercel Postgres는 무료 제공량이 넉넉해서, 이 정도 규모(하루 몇 건~몇십 건)의 사용량이면
    비용 없이 쓰실 수 있을 거예요.
